...
# CONSTRUCTOR
    ...
    def __init__(self, radius, offset, x_centroid, y_centroid):
        if type(radius) is not float:
            raise TypeError(
                "OutsideCircle.py __init__ radius - must be a float."
            )
        if radius <= 0:
            raise ValueError(
                "OutsideCircle.py __init__ radius - must be greater than 0."
            )
        if type(offset) is not float:
            raise TypeError(
                "OutsideCircle.py __init__ offset - must be a float."
            )
        if offset <= 0:
            raise ValueError(
        ...
        if radius - (offset * 2) <= 0:
            raise ValueError(
                "OutsideCircle.py __init__ offset - must be less than half the radius."
            )
        if type(x_centroid) is not float:
            raise TypeError(
                "OutsideCircle.py __init__ x_centroid - must be a float."
            )
        if type(y_centroid) is not float:
            raise TypeError(
                "OutsideCircle.py __init__ y_centroid - must be a float."
            )         
