...
# CONSTRUCTOR
    ...
    def __init__(self, width, height, offset, x_centroid, y_centroid):
        if type(width) is not float:
            raise TypeError(
                "OutsideRectangle.py __init__ width - must be a float."
            )
        if width <= 0:
            raise ValueError(
                "OutsideRectangle.py __init__ width - must be greater than 0."
            )
        if type(height) is not float:
            raise TypeError(
                "OutsideRectangle.py __init__ height - must be a float."
            )
        if height <= 0:
            raise ValueError(
                "OutsideRectangle.py __init__ height - must be greater than 0."
            )   
        ...
        if type(y_centroid) is not float:
            raise TypeError(
                "OutsideRectangle.py __init__ y_centroid - must be a float."
            )    
